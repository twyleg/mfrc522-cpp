 ...
 * Constructor.
 */
MFRC522::MFRC522(ISpi& spi, IGpio& resetPin) :
	mSpi(spi),
	mResetPin(resetPin){
...
void MFRC522::PCD_ReadRegister(uint8_t reg,		///< The register to read from. One of the PCD_Register enums.
  ...
  while (index < count) {
	...
	mask |= (1 << i);
	  ...
	  uint8_t value = mSpi.writeRead(address);
	  // Apply mask to both current value of values[0] and the new data in value.
	  values[0] = (values[index] & ~mask) | (value & mask);
	}
	else { // Normal case
	  values[index] = mSpi.writeRead(address);
	}
	index++;
  }
  values[index] = mSpi.writeRead(0);// Read the final byte. Send 0 to stop reading.
} // End PCD_ReadRegister()
